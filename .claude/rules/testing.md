...
- Use `pytest -n auto` for faster parallel execution, but limit concurrency if the running environment is resource-constrained.
- Capture Allure results every time a regression is addressed; store artifacts under `reports/allure-results`.
- Use the custom report bundle command `python utils/generate_reports.py` to generate complete Excel, HTML, and JSON reports locally or for CI.
